...
setwd("~/Rprojects/eco_genomics/eco_genomics") # set your working directory 

vcf <- read.vcfR("outputs/vcf_final.filtered.vcf.gz") # bring in your original filtered vcf file

vcf75 <- read.vcfR("outputs/vcf_final.filtered.vcf.gz")
vcf60 <- read.vcfR("outputs/vcf_final.60filtered.vcf.gz")
vcf90 <- read.vcfR("outputs/vcf_final.90filtered.vcf.gz")
...
vcf75.thin <- distance_thin(vcf75, min.distance=500) 
vcf60.thin <- distance_thin(vcf60, min.distance=500) 
vcf90.thin <- distance_thin(vcf90, min.distance=500) 
...
meta90 <- meta[meta$id %in% colnames(vcf90@gt[, -1]) , ]
...
write.vcf(vcf75.thin, "outputs/vcf_final.75filtered.thinned.vcf.gz")
write.vcf(vcf60.thin, "outputs/vcf_final.60filtered.thinned.vcf.gz")
write.vcf(vcf90.thin, "outputs/vcf_final.90filtered.thinned.vcf.gz")
...
system("gunzip -c ~/Rprojects/eco_genomics/eco_genomics/outputs/vcf_final.60filtered.thinned.vcf.gz > ~/vcf_final.60filtered.thinned.vcf")

# Now perform the conversion from uncompressed vcf to .geno format:
geno75 <- vcf2geno(input.file="/gpfs1/home/m/g/mgarvin1/vcf_final.75filtered.thinned.vcf",
                 output.file="outputs/vcf_final.75filtered.thinned.geno")

geno90 <- vcf2geno(input.file="/gpfs1/home/m/g/mgarvin1/vcf_final.90filtered.thinned.vcf",
                   output.file="outputs/vcf_final.90filtered.thinned.geno")

geno60 <- vcf2geno(input.file="/gpfs1/home/m/g/mgarvin1/vcf_final.60filtered.thinned.vcf",
                   output.file="outputs/vcf_final.60filtered.thinned.geno")

# Now we're ready to do the PCA!
CentPCA <- LEA::pca("outputs/vcf_final.75filtered.thinned.geno", scale=TRUE)
...
CentPCA <- load.pcaProject("vcf_final.75filtered.thinned.pcaProject")
...
PCA75.final <- ggplot(as.data.frame(CentPCA$projections),
       aes(x=V1, y=V2, color=meta75$region, shape=meta75$continent)) +
       geom_point(alpha=0.5) +
       labs(title="75% Filtering",x="PC1 (2.2%)",y="PC2 (1.1%)",color="Region",shape="Continent") +
...
combined_plot <- PCA60.final / PCA75.final / PCA90.final + plot_layout(guides = "collect")

combined_plot

ggsave("figures/CentPCA_PC1vPC2.pdf", width=6, height=12, units ="in")
